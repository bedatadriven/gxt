...
public abstract class AbstractStoreSelectionModel<M extends ModelData> extends BaseObservable implements
  ...
  protected void doDeselect(List<M> models, boolean supressEvent) {
    ...
    for (M m : models) {
      if (selected.remove(m)) {
        if (isModelsEqual(lastSelected, m)) {
  ...
  protected boolean isModelsEqual(M modelA, M modelB) {
    return (modelA == modelB) || (modelA != null && modelA.equals(modelB));
  }
  ...
  protected void onRemove(M model) {
    ...
    if (selected.remove(model)) {
      if (isModelsEqual(lastSelected, model)) {
        lastSelected = null;
      }
      if (isModelsEqual(getLastFocused(), model)) {
        setLastFocused(null);
      }
      fireSelectionChange();
    }
  ...
  protected void onUpdate(M model) {
    ...
    if (!isModelsEqual(model, getLastFocused()) && store.equals(model, getLastFocused())) {
      lastFocused = model;
    }
  }
